...
def test_agent_returns_empty_on_no_result_message():
    ...
    with patch("super_orchestrate.client.query", side_effect=empty_query):
        with patch("super_orchestrate.client.ResultMessage", MagicMock):
            result = client_mod.Agent(description="test", prompt="hello")
            assert result == ""
...
def test_agent_runtime_error_from_sdk_propagates():
    ...
    with patch("super_orchestrate.client.query", side_effect=bad_query):
        with patch("super_orchestrate.client.ResultMessage", MagicMock):
